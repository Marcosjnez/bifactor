double f_cfa_uls(arma::vec x, arma::mat R, int nfactors, arma::uvec indexes) {

  int n_items = R.n_rows;
  int n_parameters = x.size();
  int n_loadings = n_parameters - n_items;

  arma::mat loadings(n_items, nfactors, arma::fill::zeros);
...
arma::vec g_cfa_uls(arma::vec x, arma::mat R, int nfactors, arma::uvec indexes) {
